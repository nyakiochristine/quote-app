...
function addNightwatchTestsScriptToPackageJson(options) {
    return (tree, context) => {
        let scriptsToAdd = {};
        switch (framework_1.default(tree)) {
            case 'angular':
        ...
        util_1.addPropertyToPackageJson(tree, context, 'scripts', scriptsToAdd);
        return tree;
    };
}
...
function updateDependencies(options) {
    ...
    return (tree, context) => {
        ...
        const addDependencies = rxjs_1.of('nightwatch', '@types/node', '@types/nightwatch', driver).pipe(operators_1.concatMap((packageName) => util_1.getLatestNodeVersion(packageName)), operators_1.map((packageFromRegistry) => {
    ...
    return (tree, context) => {
        ...
        return schematics_1.chain(Object.keys(projects).map((name) => {
            ...
            return schematics_1.mergeWith(schematics_1.apply(schematics_1.url('./files'), [
                schematics_1.move(core_1.normalize(project.root)),
                schematics_1.applyTemplates(Object.assign(Object.assign(Object.assign({}, core_1.strings), options), { root: project.root ? `${project.root}/` : project.root, cucumberRunner })),
            ]));
        }))(tree, context);
    };
}
    ...
    return (tree, context) => {
        if (tree.exists('./angular.json')) {
            ...
            Object.keys(projects).forEach((project) => {
                ...
                exports.removeE2EConfig(tree, angularJsonValue, project);
                exports.removeE2ELinting(tree, angularJsonValue, project);
                context.logger.debug(`Adding e2e/tsconfig.json to angular.json-tslint config`);
                exports.addNightwatchTsConfig(tree, angularJsonValue, project);
            });
